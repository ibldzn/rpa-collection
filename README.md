...
The second argument selects `Manual` or `Automatic` for both BI and BPR. The command skips an account only when both collectability values and both current change types already match. Missing current change types fail that account. The command resolves alternate accounts first, then processes branches in code order. Accounts within a branch retain input order. One failed account does not stop the batch. A failed branch login fails that branch's accounts and processing continues. Exit status is `0` without failures, `1` with account failures, or `2` for invalid arguments or configuration. A skipped account is not a failure.
